...
const NAV_ITEMS = [
  ...
  { icon: ClipboardList, label: "Replacements", key: "replacements" },
...
function ReplacementsSection() {
  ...
  useEffect(() => {
    api.getMyReplacementRequests()
      .then((res: any) => setReplacements(res.data || []))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, []);

  if (loading) return <div className="text-center py-12 text-sm text-gray-500">Loading replacements...</div>;
  if (!replacements.length) return <div className="text-center py-16 text-gray-500/50 text-sm">No replacement requests found.</div>;
    ...
    <div className="space-y-4">
      {replacements.map((r: any) => (
        <div 
          ...
          className="sf-dash-card rounded-xl border border-gray-200 p-4 hover:border-[#E85A00] transition-colors cursor-pointer relative"
        >
          <div className="flex items-start justify-between mb-3">
            <div>
              <p className="font-semibold text-black">Order: {r.masterOrder?.orderNumber || "Unknown"}</p>
              <p className="text-xs text-gray-500 mt-1">Requested on: {formatDate(r.createdAt)}</p>
              <p className="text-sm font-medium mt-2">Reason: <span className="font-normal text-gray-600">{r.reason.replace(/_/g, ' ')}</span></p>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
            <div className="p-4 border-b flex items-center justify-between bg-gray-50">
              <h3 className="font-semibold text-lg">Replacement Details</h3>
              <button onClick={() => setSelectedReplacement(null)} className="p-1 hover:bg-gray-200 rounded-full transition-colors">
            ...
            <div className="p-5 overflow-y-auto space-y-5">
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{selectedReplacement.masterOrder?.orderNumber || "Unknown"}</p>
                </div>
                <div>
                  <p className="text-gray-500 text-xs mb-1">Status</p>
                  <p className="font-semibold">{selectedReplacement.status}</p>
                </div>
                <div>
                  <p className="text-gray-500 text-xs mb-1">Requested On</p>
                  <p className="font-medium">{formatDate(selectedReplacement.createdAt)}</p>
                </div>
                <div>
                  <p className="text-gray-500 text-xs mb-1">Reason</p>
                  <p className="font-medium">{selectedReplacement.reason.replace(/_/g, ' ')}</p>
                </div>
              </div>
              ...
              {selectedReplacement.description && (
                <div>
                  ...
                  <p className="text-sm bg-gray-50 p-3 rounded-xl border">{selectedReplacement.description}</p>
                </div>
              )}

              {selectedReplacement.reviewNotes && (
                <div>
                  <p className="text-gray-500 text-xs mb-1">Admin Note</p>
                  <p className="text-sm bg-blue-50 text-blue-900 p-3 rounded-xl border border-blue-100">{selectedReplacement.reviewNotes}</p>
                </div>
              )}

              {selectedReplacement.replacementItems && selectedReplacement.replacementItems.length > 0 && (
                <div>
                  <p className="text-gray-500 text-xs mb-2">Items to Replace</p>
                  <div className="space-y-3">
                    {selectedReplacement.replacementItems.map((item: any, idx: number) => (
                      <div key={idx} className="flex gap-3 items-center border rounded-xl p-3 bg-white">
              ...
              {selectedReplacement.images && selectedReplacement.images.length > 0 && (
                <div>
                  <p className="text-gray-500 text-xs mb-2">Attached Images</p>
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {selectedReplacement.images.map((img: string, idx: number) => (
                      <a key={idx} href={img} target="_blank" rel="noreferrer" className="shrink-0 block border rounded-lg overflow-hidden hover:border-[#E85A00] transition-colors">
    ...
    <div className="sf-dashboard flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 bg-gray-50">
          ...
          {active === "replacements" && <ReplacementsSection />}
