        ...
        <div className="max-w-7xl mx-auto px-4 py-10">
          ...
          <p className="text-sb-ink-muted/60 text-sm">
            {city
              ...
              : `Select a city to see categories available for delivery`}
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {categories.map(cat => (
              <Link
                ...
                className="group bg-sb-surface border border-sb-ink/10 rounded-2xl overflow-hidden hover:border-[#E85A00]/50 hover:shadow-[0_4px_24px_rgba(232, 90, 0,0.1)] transition-all duration-300"
              >
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <h2 className="font-bold text-sb-ink text-lg group-hover:text-[#E85A00] transition-colors">{cat.name}</h2>
                      {cat.description && (
                        <p className="text-sb-ink-muted/60 text-sm mt-1 leading-relaxed line-clamp-2">{cat.description}</p>
                      )}
                    </div>
                    <div className="w-8 h-8 rounded-lg bg-[#E85A00]/10 border border-[#E85A00]/20 flex items-center justify-center shrink-0 group-hover:bg-[#E85A00] group-hover:border-[#E85A00] transition-all">
                      <ArrowRight className="w-4 h-4 text-[#E85A00] group-hover:text-sb-on-orange transition-colors" />
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-4 pt-3 border-t border-sb-ink/10">
                    <span className="text-xs font-semibold text-[#E85A00]">Browse →</span>
                  </div>
                </div>
              </Link>
            ))}
